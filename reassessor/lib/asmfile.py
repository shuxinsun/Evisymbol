...
class AsmFileInfo:
    ...
    def get_composite_data(self, label):
        # 是否发现“复合表达式成员”（例如 label+8, symbol-addr 这种）
        # 只有出现这种，GT才认为这是“结构型数据”，而不是纯常量数组
        bHasComposite = False
        members = []    # 保存该数据对象的所有成员项（每一条 .long/.quad）
        idx = self.idx  # 记录这个 label 出现时的行号（用于之后定位）
        while self.mov_next():  # 逐行向下读
            terms = self.get_line().split()
            if terms[0] in DATA_DIRECTIVE:
                if terms[0] in ['.long', '.quad']:  # 只处理数据定义指令（.long, .quad 等）
                    # if it has label-label patterns, it would be jump table
                    if not members and '@GOTOFF' in terms[1]:   # 如果第一项是 GOT 相对引用 → 跳转表
                        self.mov_prev()
                        self.get_jmp_table(label)
                        return
                    if not members and re.search('-\.L', terms[1]): # 如果出现 label - .Lxxx → 也是跳转表
                        self.mov_prev()
                        self.get_jmp_table(label)
                        return
                    if not members and re.search('-%s'%(label), terms[1]):  # 如果出现 xxx - 当前label → 字符串表
                        self.mov_prev()
    ...
    def get_jmp_table(self, label):
        ...
        if jmp_entries:
            ...
            # ssx:按特定格式收集跳转表项
            for line in jmp_entries:
                self.my_jumptable_line.append(line[0])
