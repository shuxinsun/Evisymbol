...
class NormalizeGT:
    ...
    def __init__(self, bin_path, asm_dir, reloc_file='', build_path=''):
        """
        初始化GT符号化处理器
        ELF解析 + 重定位读取 + 符号信息收集 + 代码段反汇编 + 程序级抽象建立
    ...
    def update_table(self, addr, comp_data, asm_path):
        for line, idx in comp_data.members:
            ...
            is_jumptable_item = True if line in self.all_jumptable_lines[asm_path] else False
            data = self.comp_gen.get_data(addr, asm_path, line, idx, is_jumptable_item, value, label_dict)
    ...
    def update_data(self, addr, comp_data, asm_path):
        for line, idx in comp_data.members:
            ...
            if sz in [4,8] and re.search('.[+|-]', expr):
                ...
                #    value += self.got_addr
                is_jumptable_item = True if line in self.all_jumptable_lines[asm_path] else False
                data = self.comp_gen.get_data(addr, asm_path, line, idx , is_jumptable_item, value)
                self.prog.Data[addr] = data
    ...
    def get_objdump(self):
        ...
        # 调用 objdump 打印符号表\文件头, 提取函数符号
        # objdump -t -f 二进制文件 | grep "F .text" | sort
    ...
    def match_src_to_bin(self):
        ...
        for func_info in funcs:
            ...
            for idx, (addr, capstone_insn, asm_token) in enumerate(addressed_asm_list):
                ...
                # 处理一下asm_token，获取其操作数是否是lea、jmp、call等类型
                opcode_token = {}
                opcode_token["asm_is_call"] = capstone_insn.group(capstone.CS_GRP_CALL)
                opcode_token["asm_is_jmp"] = capstone_insn.group(capstone.CS_GRP_JUMP)
                opcode_token["asm_is_lea"] = (capstone_insn.id == capstone.x86.X86_INS_LEA)
                opcode_token["asm_is_pc_relative"] = any(
                    op.type == capstone.x86.X86_OP_MEM and op.mem.base == capstone.x86.X86_REG_RIP
                    for op in capstone_insn.operands
                )
                opcode_token["asm_is_rbp_based"] = any(
                    op.type == capstone.x86.X86_OP_MEM and op.mem.base == capstone.x86.X86_REG_RBP
                    for op in capstone_insn.operands
                )
                
                # 使用符号化生成器创建指令对象
                # 参数：地址，汇编文件路径，汇编标记，Capstone指令
                instr = self.comp_gen.get_instr(addr, asm_file.file_path, asm_token, opcode_token, capstone_insn)
                # 存储指令对象
    ...
    def find_match_func(self, func_code, func_info):
        ...
        if not ret:
            ...
            if len(ret) == 0:
                # import pdb
                # pdb.set_trace()
                print(f'[WARN] No matched assembly for {fname}')
                return None, None
    ...
    def collect_loc_candidates(self):
        ...
        for src in srcs:
            ...
            asm_file.scan()
            self.all_jumptable_lines[src] = asm_file.my_jumptable_line
    ...
    def normalize_data(self):
        """
        把“散落在汇编、符号表、重定位表里的所有数据语义”统一清洗、对齐并转成 GT 能理解的“标准符号化数据表示”，最终全部落到 Program.Data 这个总数据池里。
        ...
        for addr in self.relocs:
            ...
            data = self.comp_gen.get_data(addr, '',  asm_line, 0, False, value, r_type = r_type)
            # 将符号化数据存储到Program对象中
            self.prog.Data[addr] = data
