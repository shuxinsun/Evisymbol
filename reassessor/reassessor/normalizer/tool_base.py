...
class NormalizeTool:
    ...
    def mapper(self, map_func):
        ...
        print(self.reassem_path) # output:./output/reassem/ddisasm.s
        # ../../output/errors/ddisasm
        ...
        with open("./new_ssx/output/ddisasm_symbolization_result/address_lines.txt", "w") as file:
    	    for item in addressed_lines:
                file.write("%s\n" % str(item))
        with open("./new_ssx/output/ddisasm_symbolization_result/ReasmLabel_address_lines.txt", "w") as file:
    	    for item in addressed_lines:
    	    	if isinstance(item, ReasmLabel):
                    file.write("%s\n" % str(item))
        with open("./new_ssx/output/ddisasm_symbolization_result/ReasmInst_address_lines.txt", "w") as file:
            for item in addressed_lines:
            	if isinstance(item, ReasmInst):
                    file.write("%s\n" % str(item))
        with open("./new_ssx/output/ddisasm_symbolization_result/ReasmData_address_lines.txt", "w") as file:
            for item in addressed_lines:
            	if isinstance(item, ReasmData):
                    file.write("%s\n" % str(item))
        with open("./new_ssx/output/ddisasm_symbolization_result/ReasmSetLabel_address_lines.txt", "w") as file:
            for item in addressed_lines:
            	if isinstance(item, ReasmSetLabel):
                    file.write("%s\n" % str(item))
