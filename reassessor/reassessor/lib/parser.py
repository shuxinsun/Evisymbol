...
class CompGen:
    ...
    def get_data(self, addr, asm_path, line, idx , is_jumptable_item=None, value=0, additional_dict=None, r_type=None):
        """
        处理数据定义指令，提取符号信息
        ...
        # 创建数据类型对象，包含符号化信息
        return DataType(addr, asm_path, line, idx, factors, r_type = r_type, is_jumptable_item=is_jumptable_item)

    def get_inst(self, addr, asm_path, opcode_token, asm_token, insn):
        """
        处理指令，提取符号信息
        """
        # 处理跳转和调用指令
    ...
    def rearrange_operands(self, addr, asm_path, opcode_token, asm_token, insn):
        #op_str_list = []
        """
        ...
        if insn.group(capstone.CS_GRP_JUMP) or insn.group(capstone.CS_GRP_CALL):
            ...
            if factors.has_label():
                # 如果有标签，创建带立即数符号的指令类型
                return InstType(addr, asm_path, opcode_token, asm_token, imm = factors)
            return InstType(addr, asm_path, opcode_token, asm_token)
        ...
        return InstType(addr, asm_path, opcode_token, asm_token, disp=disp, imm=imm)
    ...
    def get_instr(self, addr, asm_path, asm_token, opcode_token=None, insn=None):
        """
        获取指令的符号化表示
        """
        ...
        # 使用Capstone IR（如果有的话）
        if insn:
            return self.rearrange_operands(addr, asm_path, opcode_token, asm_token, insn)
